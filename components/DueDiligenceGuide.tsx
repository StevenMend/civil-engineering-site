  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className="w-full max-w-6xl h-[85vh] overflow-hidden relative mx-auto"
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.95, opacity: 0 }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Toast - Flexbox centered positioning */}
            <AnimatePresence>
              {status.message && (
                <div className="fixed top-6 left-0 right-0 z-[60] flex justify-center px-4">
                  <motion.div
                    ...
                    className={`px-6 py-4 rounded-xl text-sm font-medium text-center backdrop-blur-md border shadow-2xl min-w-[300px] max-w-[90vw] ${
                      status.type === 'success' 
                        ? 'bg-emerald-500/90 text-white border-emerald-500/50 shadow-emerald-500/25' 
                        : 'bg-red-500/90 text-white border-red-500/50 shadow-red-500/25'
                    }`}
                  >
                    <div className="flex items-center justify-center gap-3">
                      {status.type === 'success' && <CheckCircle className="w-5 h-5" />}
                      <span className="break-words font-medium">{status.message}</span>
                    </div>
                  </motion.div>
                </div>
              )}
            ...
            {/* Background Effects - Same as ContactSection */}
            ...
            {/* Container with same styling as ContactSection */}
            <div className="relative h-full">
              {/* Background Elements - Same pattern */}
              ...
              {/* Neon border effect - Same as ContactSection */}
              ...
              {/* Header - Fixed height */}
              <div className="relative flex items-center justify-between p-6 border-b border-white/10 h-[80px] flex-shrink-0">
                <div>
                  <h1 className="text-xl md:text-2xl font-semibold text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.9)]">
                    {getTerrainTranslation('dueDiligenceGuide.title')}
                  </h1>
                  <p className="text-gray-200 text-sm mt-1 drop-shadow-[0_0_6px_rgba(255,255,255,0.7)]">
              ...
              {/* Main Content - Responsive Layout with calculated height */}
              <div className="relative flex flex-col lg:flex-row h-[calc(100%-80px)]">
                {/* Left Side - Guide Content */}
                <div className="flex-1 overflow-y-auto p-6 lg:h-full">
                  <div className="space-y-4">
                    {sections.map((section, index) => {
                      ...
                      return (
                        <motion.div
                          key={index}
                          className="bg-zinc-900/30 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition-all duration-300"
                ...
                {/* Right Side - Contact Form & Proposal */}
                <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-white/10 overflow-y-auto lg:h-full">
                  <form onSubmit={handleSubmit} className="relative space-y-4 lg:space-y-6 p-4 lg:p-6 h-full" noValidate>

                    <div className="mb-4 lg:mb-6">
                      <h3 className="text-base lg:text-lg font-medium text-white mb-2 drop-shadow-[0_0_6px_rgba(255,255,255,0.7)]">
                        {getTerrainTranslation('dueDiligenceGuide.contactForm.title')}
                      </h3>
                      <p className="text-gray-200 text-xs mb-3 lg:mb-4 drop-shadow-[0_0_4px_rgba(255,255,255,0.5)]">
                    ...
                    <div className="space-y-2 lg:space-y-3">
                      ...
                      <Input
                        ...
                        className={`bg-zinc-900 border-zinc-700 focus:border-white transition-all duration-300 pl-4 h-10 lg:h-12 text-sm relative ${
                    ...
                    <div className="space-y-2 lg:space-y-3">
                      ...
                      <Input
                        ...
                        className={`bg-zinc-900 border-zinc-700 focus:border-white transition-all duration-300 pl-4 h-10 lg:h-12 text-sm ${
                    ...
                    {/* Submit Button with Easy Chat option */}
                    <div className="pt-2 space-y-2 lg:space-y-3">
                      <Button 
                        type="submit" 
                        disabled={isLoading}
                        className="w-full angular-button disabled:opacity-50 disabled:cursor-not-allowed h-10 lg:h-12 !flex !items-center !justify-center !py-0 text-sm"
                      >
                      ...
                      <button
                        ...
                        className="w-full bg-green-600 hover:bg-green-700 text-white py-2 lg:py-2.5 px-4 rounded-lg font-medium transition-all duration-300 flex items-center justify-center gap-2 text-sm border border-green-500/30 hover:border-green-500/50"
                      >
                    ...
                    {proposalData && (
                      <div className="bg-zinc-800/30 border border-white/10 rounded-lg p-3 lg:p-4 mt-4 lg:mt-6">
